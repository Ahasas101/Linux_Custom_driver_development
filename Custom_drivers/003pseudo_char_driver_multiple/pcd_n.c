...
struct pcdrv_private_data pcdrv_data = 
{
	...
	.pcdev_data = {
		[0] = {
			...
			.serial_number = "PCDEV1XYZ121",
			.perm = RDONLY
			},
		[1] = {
			.buffer = device_buffer_pcdev2,
			.size = MEM_SIZE_MAX_PCDDEV2,
			.serial_number = "PCDEV1XYZ122",
			.perm = WRONLY
			},
		[2] = {
...
int pcd_open (struct inode *inode, struct file *filp)
{
	...
	/*First we need to find out on which device file open was attempted by the user space*/
	minor_n = MINOR(inode->i_rdev);
	pr_info("Minor access = %d\n",minor_n);
	
	/*then we get device's private data structure*/
	pcdev_data = container_of(inode->i_cdev, struct pcdev_private_data, cdev);
	/*to supply device private data to other methods of the driver like read and write*/
